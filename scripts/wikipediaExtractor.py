...
selfClosing_tag_patterns = [
    re.compile(r'<\s*%s\b[^>]*/\s*>' % tag, re.DOTALL | re.IGNORECASE) for tag
    in selfClosingTags
]
...
def makeInternalLink(title, label):
    ...
    if Extractor.keepLinks:
        return '<a href="%s">%s</a>' % (
        urllib.quote(title.encode('utf-8')), label)
    else:
        return label
...
def makeExternalLink(url, anchor):
    ...
    if Extractor.keepLinks:
        return '<a href="%s">%s</a>' % (
        urllib.quote(url.encode('utf-8')), anchor)
    else:
        return anchor
