...
def test_workflow_easy():
    ...
    with TempIndex(schema, "easyhilite") as ix:
        with ix.writer() as w:
...
def test_workflow_manual():
    schema = fields.Schema(id=fields.ID(stored=True),
                           title=fields.TEXT(stored=True))
    with TempIndex(schema, "manualhilite") as ix:
        with ix.writer() as w:
            w.add_document(id=u"1", title=u"The man who wasn't there")
            w.add_document(id=u"2", title=u"The dog who barked at midnight")
            w.add_document(id=u"3", title=u"The invisible man")
...
def test_unstored():
    schema = fields.Schema(text=fields.TEXT(vector=True), tags=fields.KEYWORD)
    with TempIndex(schema, "hiliteunstored") as ix:
        with ix.writer() as w:
...
def test_multifilter():
    ...
    with TempIndex(schema, "hilitemultifilter") as ix:
        with ix.writer() as w:
...
def test_pinpoint():
    ...
    with TempIndex(schema, "hilitepinpoint") as ix:
        with ix.writer() as w:
...
def test_highlight_wildcards():
    ...
    with TempIndex(schema, "hilitewild") as ix:
        with ix.writer() as w:
...
def test_highlight_ngrams():
    ...
    with TempIndex(schema, "hilitegrams") as ix:
        with ix.writer() as w:
...
def test_whole_noterms():
    ...
    with TempIndex(schema, "hilitewholenoterms") as ix:
        with ix.writer() as w:
