...
def test_add_text():
    with TempIndex(schema, "classifyaddtext") as ix:
        create_index(ix)
        with ix.reader() as r:
...
def test_keyterms():
    with TempIndex(schema, "keyterms") as ix:
        ...
        with ix.searcher() as s:
            ...
            keys = list(s.key_terms([docid], "content", numterms=3))
            target = [u"collision", u"calculations", u"damped"]
            assert [t[0] for t in keys] == target


def test_keyterms_from_text():
    with TempIndex(schema, "keytermsfromtext") as ix:
        create_index(ix)
        with ix.searcher() as s:
...
def test_more_like_this():
    ...
    with TempIndex(schema, "morelikethis") as ix:
        with ix.writer() as w:
