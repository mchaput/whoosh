...
def test_daterange_empty_field():
    ...
    with TempIndex(schema, "daterangeempty") as ix:
        with ix.writer() as w:
            w.add_document(test=None)
...
def test_prefix_plugin():
    ...
    with TempIndex(schema, "prefixplugin") as ix:
        with ix.writer() as w:
...
def test_copyfield():
    ...
    target = text_type(qp.parse(u"spruce view"))
    assert target == "((name:spruce OR name_phone:SPRS) AND (name:view OR name_phone:F OR name_phone:FF))"
...
def test_fuzzy_prefix():
    ...
    schema = fields.Schema(title=fields.STORED, content=fields.TEXT)
    with TempIndex(schema, "fuzzyprefix") as ix:
        with ix.writer() as w:
            w.add_document(title="a", content=u"1111")  # Match
            w.add_document(title="b", content=u"0000")  # No match
            w.add_document(title="c", content=u"1001")  # Match (2 edits)
            w.add_document(title="d", content=u"foo")  # Match (foo)
            w.add_document(title="e", content=u"0111")  # No match (prefix)

        parser = QueryParser("content", ix.schema)
        parser.add_plugin(FuzzyTermPlugin())
        q = parser.parse("1111~2/1 OR foo")

        assert isinstance(q, query.Or)
        ft = q[0]
        assert isinstance(ft, query.FuzzyTerm)
        assert ft.maxdist == 2
        assert ft.prefixlength == 1

        with ix.searcher() as s:
            results = s.search(q)
            assert results.total_length() == 3
            assert results.scored_length() == 3
            ids = "".join(sorted(hit["title"] for hit in results))
            assert ids == "acd"
