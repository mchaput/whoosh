...
def test_scoring():
    ...
    with ix.searcher() as s:
        q = query.Nested(query.Term("kind", "class"), query.Term("name", "add"))
        r = s.search(q)
        assert_equal([hit["name"] for hit in r], ["Calculator", "Index", "Accumulator"])
...
def test_deletion():
    ...
    with ix.searcher() as s:
        ...
        r = s.search(q)
        assert_equal([hit["name"] for hit in r], ["Calculator", "Index", "Accumulator", "Deleter"])
