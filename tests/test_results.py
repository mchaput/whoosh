...
def test_score_retrieval():
    schema = fields.Schema(t=fields.TEXT(stored=True),
                           c=fields.TEXT(stored=True))
    with TempIndex(schema, "scoreretrieve") as ix:
        with ix.writer() as w:
            w.add_document(
                t=u"Miss Mary",
                c=u"Mary had a little white lamb its fleece was white as snow"
            )
            w.add_document(
                t=u"Snow White",
                c=u"Snow white lived in the forest with seven dwarfs"
            )

        with ix.searcher() as s:
            results = s.search(query.Term("c", "white"))
            assert results.total_length() == 2
            assert results[0]['t'] == u"Miss Mary"
            assert results[1]['t'] == u"Snow White"
...
def test_resultcopy():
    ...
    with TempIndex(schema, "resultcopy") as ix:
        with ix.writer() as w:
...
def test_resultslength():
    schema = fields.Schema(id=fields.ID(stored=True),
                           value=fields.TEXT)
    with TempIndex(schema, "resultlen") as ix:
        with ix.writer() as w:
...
def test_combine():
    schema = fields.Schema(id=fields.ID(stored=True),
                           value=fields.TEXT)
    with TempIndex(schema, "resultcombine") as ix:
        with ix.writer() as w:
...
def test_results_filter():
    ...
    with TempIndex(schema, "resultfilter") as ix:
        with ix.writer() as w:
...
def test_extend_empty():
    ...
    with TempIndex(schema, "extendempty") as ix:
        with ix.writer() as w:
...
def test_extend_filtered():
    ...
    with TempIndex(schema, "extendfiltered") as ix:
        with ix.writer() as w:
...
def test_pages():
    ...
    with TempIndex(schema, "resultpages") as ix:
        with ix.writer() as w:
...
def test_pages_with_filter():
    ...
    schema = fields.Schema(id=fields.ID(stored=True),
                           type=fields.TEXT(),
                           c=fields.TEXT)
    with TempIndex(schema, "resultpagesfilter") as ix:
        with ix.writer() as w:
...
def test_extra_slice():
    ...
    with TempIndex(schema, "extraslice") as ix:
        with ix.writer() as w:
...
def test_page_counts():
    ...
    schema = fields.Schema(id=fields.ID(stored=True))
    with TempIndex(schema, "pagecounts") as ix:
        with ix.writer() as w:
            for i in xrange(10):
...
def test_resultspage():
    ...
    with TempIndex(schema, "resultspage") as ix:
        with ix.writer() as w:
        ...
        with ix.searcher() as s:
            ...
            assert rp.total_length() == 0
            assert rp.is_last_page()


def test_highlight_setters():
    schema = fields.Schema(text=fields.TEXT)
    with TempIndex(schema, "hilitesetters") as ix:
        with ix.writer() as w:
...
def test_snippets():
    ...
    with TempIndex(schema, "snippets") as ix:
        with ix.writer() as w:
        ...
        with ix.searcher() as s:
            ...
            his = [hit.highlights("text", top=1) for hit in r]
            assert sorted(his) == sorted(target)
...
def test_keyterms():
    ...
    with TempIndex(schema, "keyterms") as ix:
        with ix.writer() as w:
            w.add_document(path=u"a", content=u"This is some content")
...
def test_lengths():
    ...
    with TempIndex(schema, "resultlengths") as ix:
        with ix.writer() as w:
...
def test_lengths2():
    ...
    with TempIndex(schema, "resultlengths2") as ix:
        count = 0
        with ix.writer() as w:
...
def test_stability():
    ...
    with TempIndex(schema, "resultstable") as ix:
        with ix.writer() as w:
...
def test_terms():
    schema = fields.Schema(text=fields.TEXT(stored=True))
    with TempIndex(schema, "resultterms") as ix:
        with ix.writer() as w:
...
def test_hit_column():
    # Not stored
    schema = fields.Schema(text=fields.TEXT())
    with TempIndex(schema, "hitcolumn") as ix:
        with ix.writer() as w:
...
def test_closed_searcher():
    ...
    with TempIndex(schema, "closedsearcher") as ix:
        with ix.writer() as w:
...
def test_paged_highlights():
    schema = fields.Schema(text=fields.TEXT(stored=True))
    with TempIndex(schema, "pagedhilites") as ix:
        with ix.writer() as w:
...
def test_phrase_keywords():
    schema = fields.Schema(text=fields.TEXT(stored=True))
    with TempIndex(schema, "phrasekeywords") as ix:
        with ix.writer() as w:
...
def test_every_keywords():
    ...
    with TempIndex(schema, "everykeywords") as ix:
        with ix.writer() as w:
