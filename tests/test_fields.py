...
def test_declarative():
    ...
    schema = fields.ensure_schema(MySchema)
    assert schema.names() == ["content", "date", "path", "title"]

    schema = fields.ensure_schema(MySchema())
    assert schema.names() == ["content", "date", "path", "title"]

    with pytest.raises(fields.FieldConfigurationError):
        schema = fields.ensure_schema(object())
...
def test_index_numeric():
    ...
    with TempIndex(schema, "indexnumeric") as ix:
        with ix.writer() as w:
...
def test_numeric():
    ...
    with TempIndex(schema, "numeric") as ix:
        with ix.writer() as w:
...
def test_decimal_numeric():
    ...
    with TempIndex(schema, "decimalnumeric") as ix:
...
def test_numeric_ranges():
    ...
    with TempIndex(schema, "numericranges") as ix:
        with ix.writer() as w:
            for i in xrange(400):
...
def test_numeric_ranges_unsigned():
    ...
    with TempIndex(schema, "numericranges_u") as ix:
        with ix.writer() as w:
            for v in values:
...
def test_decimal_ranges():
    ...
    with TempIndex(schema, "decimalranges") as ix:
        with ix.writer() as w:
            count = Decimal("0.0")
            inc = Decimal("0.2")
            for _ in xrange(500):
...
def test_numeric_errors():
    ...
    with pytest.raises(ValueError):
        _, posts = f.index(-32769)
        list(posts)
    with pytest.raises(ValueError):
        _, posts = f.index(32768)
        list(posts)
...
def test_nontext_document():
    ...
    with TempIndex(schema, "nontextdoc") as ix:
        dt = datetime.now()
        with ix.writer() as w:
            for i in xrange(50):
...
def test_nontext_update():
    ...
    with TempIndex(schema, "nontextupdate") as ix:
        dt = datetime.now()
        with ix.writer() as w:
            for i in xrange(10):
                w.add_document(id=i, num=i, date=dt + timedelta(days=i))
...
def test_datetime():
    ...
    with TempIndex(schema, "datetimefield") as ix:
        with ix.writer() as w:
...
def test_boolean():
    ...
    with TempIndex(schema, "booleanfield") as ix:
        with ix.writer() as w:
...
def test_boolean2():
    ...
    with TempIndex(schema, "boolean2") as ix:
        with ix.writer() as w:
...
def test_boolean3():
    schema = fields.Schema(t=fields.TEXT(stored=True, field_boost=5),
                           b=fields.BOOLEAN(stored=True),
                           c=fields.TEXT)
    with TempIndex(schema, "boolean3") as ix:
        with ix.writer() as w:
...
def test_boolean_find_deleted():
    ...
    schema = fields.Schema(i=fields.STORED, b=fields.BOOLEAN(stored=True))
    count = 0
    with TempIndex(schema, "booleandeleted") as ix:
        with ix.writer() as w:
            for c in domain:
        ...
        with ix.writer() as w:
            w.delete_by_term("b", "true")

        with ix.searcher() as s:
            # Double check that documents with b=True are all deleted
            reader = s.reader()
            for docid, stored in reader.all_stored_fields():
                assert not stored.get("b")
...
def test_boolean_multifield():
    ...
    with TempIndex(schema, "booleanmulti") as ix:
        with ix.writer() as w:
...
def test_missing_field():
    ...
    with TempIndex(schema, "missingfield") as ix:
        with ix.searcher() as s:
...
def test_token_boost():
    ...
    assert length == 3
    assert [p.id for p in posts] == [b'F', b'FF', b'SPRS']
    assert [p.weight for p in posts] == [1.0, 0.5, 1.0]


def test_subfields():
    schema = fields.Schema(t=fields.ReverseField(fields.TEXT()))
    with TempIndex(schema) as ix:
        with ix.writer() as w:
            w.add_document(t=u"alfa bravo charlie")

        with ix.reader() as r:
            assert list(r.lexicon("t")) == [b"alfa", b"bravo", b"charlie"]
            assert list(r.lexicon("rev_t")) == [b"afla", b"eilrahc", b"ovarb"]
