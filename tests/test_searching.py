...
def test_not():
    q = query.And([
        query.Or([
            query.Term("value", u"red"),
            query.Term("name", u"yellow")
        ]),
        query.Not(
            query.Term("name", u"quick")
        )
    ])
    _run_query(q, [u"A", u"E"])


def test_topnot():
    _run_query(query.Not(query.Term("value", "red")), [u"B", "C", "E"])
    _run_query(query.Not(query.Term("name", "yellow")),
               [u"B", u"C", u"D"])
...
def test_not2():
    ...
    with TempIndex(schema) as ix:
        with ix.writer() as w:
            w.add_document(name=u"a", value=u"alfa bravo charlie delta echo")
            w.add_document(name=u"b", value=u"bravo charlie delta echo foxtrot")
            w.add_document(name=u"c", value=u"charlie delta echo foxtrot golf")
...
def test_posting_phrase():
    ...
    with TempIndex(schema) as ix:
        with ix.writer() as w:
            ...
            w.add_document(name=u"C", value=u"Miss Little Muffet tuffet sat")
        ...
        class CommentWeighting(WeightingModel):
            ...
            class CommentScorer(Scorer):
                ...
                def score(self, matcher):
                    ...
                    ncomments = float(sf.get("n_comments", 0))
                    return 1 - 1 / ncomments
...
def test_fuzzyterm2():
    ...
    with TempIndex(schema) as ix:
        ...
        with ix.searcher() as s:
            assert (list(s.reader().terms_within("f", u"brave", 1))
                    == ["bravo"])
            q = query.FuzzyTerm("f", "brave")
            assert [d["id"] for d in s.search(q)] == [1, 2]
...
def test_collapse():
    # id, text, size, tag
    domain = [(u"a", u"blah blah blah", 5, u"x"),
              (u"b", u"blah", 3, u"y"),
              (u"c", u"blah blah blah blah", 2, u"z"),
              (u"d", u"blah blah", 4, u"x"),
              (u"e", u"bloop", 1, u"-"),
              (u"f", u"blah blah blah blah blah", 6, u"x"),
              (u"g", u"blah", 8, u"w"),
              (u"h", u"blah blah", 7, u"=")]
    ...
    with TempIndex(schema) as ix:
        ...
        with ix.searcher() as s:
            ...
            col = s.q.with_query(q).top()
            r = col.results()
            assert " ".join(hit["id"] for hit in r) == "f c a d h b g"

            col = s.q.with_query(q).top(3).collapse("tag")
            r = col.results()
            assert " ".join(hit["id"] for hit in r) == "f c h"

            col = s.q.with_query(q).top().collapse("tag")
            r = col.results()
            ...
            eq = query.Every()
            col = s.q.with_query(eq).sorted_by("size").collapse("tag")
            r = col.results()
...
def test_collapse_length():
    domain = (u"alfa apple agnostic aplomb arc "
              u"bravo big braid beer "
              u"charlie crouch car "
              u"delta dog "
              u"echo "
              u"foxtrot fold flip "
              u"golf gym goop"
...
def test_collapse_order():
    ...
    with TempIndex(schema) as ix:
        ...
        with ix.searcher() as s:
            ...
            check(dict(sortedby=price, collapse=tag), "h b c e")
...
def test_score_length():
    ...
    with TempIndex(schema) as ix:
        ...
        with ix.writer() as w:
            ...
            # corresponding document in the first segment, the field doesn't
            # exist, so if the scorer is getting segment offsets wrong, scoring
            # this document will error
...
def test_issue_334():
    ...
    with TempIndex(schema) as ix:
        ...
        with ix.searcher() as s:
            ...
            q = query.And([
                query.NestedChildren(pq, cq),
                query.Term('returns', 'void')
            ])
            r = s.search(q)
            assert len(r) == 1
...
def test_results_order():
    ...
    words = u"alfa bravo charlie delta echo"
    ...
    with TempIndex(schema) as ix:
        ...
        with ix.searcher() as s:
            q = query.Or([query.Term("text", word) for word in words.split()])
            r1 = s.search(q, limit=None)
            r2 = s.search(q, limit=10)
            ids1 = [hit["id"] for hit in r1]
            ids2 = [hit["id"] for hit in r2]
            assert ids1 == ids2
