...
def test_max_field_length():
    ...
    with TempIndex(schema, "maxfieldlen") as ix:
        for i in xrange(1, 200, 7):
            with ix.writer() as w:
                w.add_document(t=u"word " * i)
...
def test_minmax_field_length():
    schema = fields.Schema(t=fields.TEXT)
    with TempIndex(schema, "minmaxfieldlen") as ix:
        least = 999999
        most = 0
...
def test_replacements():
    ...
    a = matching.ListMatcher([
        Posting(id=1, weight=0.25, length=1),
        Posting(id=2, weight=0.25, length=1),
        Posting(id=3, weight=0.25, length=1),
    ], scorer=sc)
    b = matching.ListMatcher([
        Posting(id=1, weight=0.25, length=1),
        Posting(id=2, weight=0.25, length=1),
        Posting(id=3, weight=0.25, length=1),
    ], scorer=sc)
    ...
    ls1 = matching.ListMatcher([
        Posting(id=1, weight=0.1, length=1),
        Posting(id=2, weight=0.1, length=1),
        Posting(id=3, weight=0.1, length=1),
    ], scorer=scoring.WeightScorer(0.1))
    ls2 = matching.ListMatcher([
        Posting(id=1, weight=0.2, length=1),
        Posting(id=2, weight=0.2, length=1),
        Posting(id=3, weight=0.2, length=1),
    ], scorer=scoring.WeightScorer(0.2))
    ls3 = matching.ListMatcher([
        Posting(id=1, weight=0.3, length=1),
        Posting(id=2, weight=0.3, length=1),
        Posting(id=3, weight=0.3, length=1),
