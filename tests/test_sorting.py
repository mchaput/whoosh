...
def test_sortedby():
    ...
    try_sort("id", get_id)
    try_sort("id", get_id, limit=5)
    try_sort("id", get_id, reverse=True)
    try_sort("id", get_id, limit=5, reverse=True)
...
def test_empty_field():
    ...
    with TempIndex(schema, "emptysort") as ix:
        ...
        with ix.searcher() as s:
            r = s.q.all().sorted_by("key").results()
...
def test_function_facet():
    ...
    with TempIndex(schema) as ix:
        ...
        def fn(searcher, docnum):
            v = searcher.vector(docnum, "text")
            aweight = v.weight_for(b"alfa")
            bweight = v.weight_for(b"bravo")

            # Give high score to documents that have equal number of "alfa"
            # and "bravo". Negate value so higher values sort first

            return 0 - (1.0 / (abs(aweight - bweight) + 1.0))
...
def test_numeric_field_facet():
    ...
    with TempIndex(schema) as ix:
        ...
        with ix.searcher() as s:
            ...
            mf = mf.add_field("v1").add_field("v2", reverse=True)
...
def test_query_facet():
    ...
    with TempIndex(schema) as ix:
        ...
        with ix.searcher() as s:
            ...
            r = s.q.all()\
                .grouped_by(facet, name="initials")\
                .results()
...
def test_overlapping_vector():
    schema = fields.Schema(id=fields.STORED,
                           tags=fields.KEYWORD(scorable=True, vector=True))
    with TempIndex(schema) as ix:
        with ix.writer() as w:
...
def test_sort_filter():
    ...
    with TempIndex(schema, "sortfilter") as ix:
        ...
        with ix.searcher() as s:
            ...
            correct = [d for d in source if d["group"] == "bravo"][:20]
            docs = [h.fields() for h in r]
            assert docs == correct
            ...
            correct = [d for d in source if d["group"] == "bravo"]
            assert [h.fields() for h in r] == correct
...
def test_translate():
    domain = [(u"alfa", 100, 50), (u"bravo", 20, 80), (u"charlie", 10, 10),
              (u"delta", 82, 39), (u"echo", 20, 73), (u"foxtrot", 81, 59),
              (u"golf", 39, 93), (u"hotel", 57, 48), (u"india", 84, 75),
              ]
    ...
    with TempIndex(schema) as ix:
        ...
        with ix.searcher() as s:
            ...
            r = s.q.all().sorted_by("a").results()
            target = "charlie bravo echo golf hotel foxtrot delta india alfa"
            assert " ".join([hit["name"] for hit in r]) == target
            ...
            r = s.q.all().sorted_by(tf).results()
            ...
            target = [x[0] for x
                      in sorted(domain, key=lambda x: (x[1] + x[2]) / 2)]
...
def test_nocachefield_segments():
    ...
    with TempIndex(schema) as ix:
        ...
        with ix.searcher() as s:
            ...
            target = ["juliet", "india", "foxtrot", "delta", "charlie", "bravo"]
            assert [hit["a"] for hit in r] == target

            mq = query.Or([query.Term("a", u"bravo"),
                           query.Term("a", u"delta")])
            anq = query.AndNot(q, mq)
            r = s.search(anq, sortedby=facet)
            target = ["juliet", "india", "foxtrot", "charlie"]
            assert [hit["a"] for hit in r] == target
            ...
            fq = query.Or([query.Term("a", u"alfa"),  # in seg 2
                           query.Term("a", u"charlie"),  # in seg 1
                           query.Term("a", u"echo"),  # deleted (was in seg 0)
                           query.Term("a", u"india"),  # in seg 2
                           ])
            r = s.q.all().filter(include=fq).sorted_by(facet).results()
            ...
            target = ["kilo", "juliet", "foxtrot", "delta", "charlie", "bravo"]
            assert [hit["a"] for hit in r] == target


def test_groupby_phrase():
    domain = {u"Alan Ball": u"Tel Aviv", u"Alan Charles": u"San Francisco",
              u"Alan Darwin": u"London", u"Alan Eames": u"Paris"}
    ...
    with TempIndex(schema) as ix:
        with ix.writer() as w:
            for name, city in domain.items():
                w.add_document(name=name, city=city, city_g=city)
...
def test_sort_text_field():
    domain = ((u"Visual Display of Quantitative Information, The", 10),
              (u"Envisioning Information", 10),
              (u"Visual Explanations", 10),
              (u"Beautiful Evidence", -10),
              (u"Visual and Statistical Thinking", -10),
              (u"Cognitive Style of Powerpoint", -10))
    ...
    with TempIndex(schema) as ix:
        with ix.writer() as w:
            for title, num in domain:
                w.add_document(title=title, num=num)
        test(ix)
...
#
#         with ix.writer() as w:
#             sorting.add_sortable(w, "chapter",
#                                  sorting.StoredFieldFacet("chapter"))
...
def test_missing_column():
    ...
    with TempIndex(schema) as ix:
        ...
        with ix.searcher() as s:
            ...
            # Make sure we're using a ColumnCategorizer to do the sorting
            c = s.q.with_query(q).sorted_by("age")
            assert isinstance(c, collectors.SortingCollector)
            r = c.results()
            assert isinstance(c._catter, sorting.ColumnCategorizer)
            assert [hit["id"] for hit in r] == [0, 6, 5, 7]

            r = s.search(q, sortedby="age", reverse=True)
            assert [hit["id"] for hit in r] == [7, 5, 6, 0]
