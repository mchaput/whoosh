...
def test_regextokenizer():
    ...
    rex = tokenizers.RegexTokenizer("[A-Z]+")
    assert [t.text for t in rex(value)] == ["AAA", "BBB", "CCC", "DDD"]

    rex = tokenizers.RegexTokenizer("[A-Z]+", gaps=True)
    assert [t.text for t in rex(value)] == ["aaa", "bbb", "ccc", "ddd"]


def test_path_tokenizer():
    value = u"/alfa/bravo/charlie/delta/"
    pt = tokenizers.PathTokenizer()
...
def test_path_tokenizer2():
    path_field = fields.TEXT(analyzer=tokenizers.PathTokenizer())
    schema = fields.Schema(path=path_field)
    with TempIndex(schema) as index:
...
def test_composition1():
    ca = tokenizers.RegexTokenizer() | filters.LowercaseFilter()
...
def test_composition3():
    sa = tokenizers.RegexTokenizer() | filters.StopFilter()
    assert sa.__class__.__name__ == "CompositeAnalyzer"
...
def test_shared_composition():
    shared = tokenizers.RegexTokenizer(r"\S+") | filters.LowercaseFilter()

    ana1 = shared | ngrams.NgramFilter(3)
    ana2 = shared | morph.DoubleMetaphoneFilter()
...
def test_multifilter():
    ...
    mf = filters.MultiFilter(a=f1, b=f2)
    ana = tokenizers.RegexTokenizer(r"\S+") | mf
...
def test_tee_filter():
    ...
    f1 = filters.LowercaseFilter()
    f2 = filters.ReverseTextFilter()
    ana = tokenizers.RegexTokenizer(r"\S+") | filters.TeeFilter(f1, f2)
    ...
    f2 = filters.ReverseTextFilter() | ucfilter()
    ana = tokenizers.RegexTokenizer(r"\S+") | filters.TeeFilter(f1, f2)
    result = " ".join([t.text for t in ana(target)])
    assert result == "alfa AFLA bravo OVARB charlie EILRAHC"

    f1 = filters.PassFilter()
    f2 = intraword.BiWordFilter()
    ana = (tokenizers.RegexTokenizer(r"\S+") |
           filters.TeeFilter(f1, f2) |
...
def test_intraword():
    iwf = intraword.IntraWordFilter(mergewords=True, mergenums=True)
    ana = tokenizers.RegexTokenizer(r"\S+") | iwf
...
def test_word_segments():
    ...
    cwf = intraword.CompoundWordFilter(wordset, keep_compound=True)
    ana = tokenizers.RegexTokenizer(r"\S+") | cwf
    ...
    cwf = intraword.CompoundWordFilter(wordset, keep_compound=False)
    ana = tokenizers.RegexTokenizer(r"\S+") | cwf
    target = u"alfacharlie bravodelta delto bravo subalfa"
    tokens = [t.text for t in ana(target)]
...
def test_double_metaphone():
    ...
    mf = (tokenizers.RegexTokenizer() |
          filters.LowercaseFilter() |
          morph.DoubleMetaphoneFilter())
    results = [(t.text, t.boost) for t in mf(u"Spruce View")]
    assert results == [('SPRS', 1.0), ('F', 1.0), ('FF', 0.5)]

    mf = (tokenizers.RegexTokenizer() |
          filters.LowercaseFilter() |
          morph.DoubleMetaphoneFilter(combine=True))
    results = [(t.text, t.boost) for t in mf(u"Spruce View")]
...
def test_substitution():
    mf = tokenizers.RegexTokenizer(r"\S+") | filters.SubstitutionFilter("-", "")
    assert ([t.text for t in mf(u"one-two th-re-ee four")]
            == ["onetwo", "threee", "four"])

    mf = (tokenizers.RegexTokenizer(r"\S+") |
          filters.SubstitutionFilter("([^=]*)=(.*)", r"\2=\1"))
    assert [t.text for t in mf(u"a=b c=d ef")] == ["b=a", "d=c", "ef"]


def test_delimited_attribute():
    ana = tokenizers.RegexTokenizer(r"\S+") | filters.DelimitedAttributeFilter()
...
def test_url():
    sample = (
        u"Visit http://bitbucket.org/mchaput/whoosh or "
        u"urn:isbn:5930502 or http://www.apple.com/."
    )

    anas = [analyzers.SimpleAnalyzer(filters.url_pattern),
            analyzers.StandardAnalyzer(filters.url_pattern, stoplist=None)]
    for ana in anas:
...
def test_name_field():
    ...
    ana = (tokenizers.RegexTokenizer(r"\S+") |
           filters.LowercaseFilter() |
           morph.DoubleMetaphoneFilter(combine=True))
    namefield = fields.TEXT(analyzer=ana)
    namefield.multitoken_query = "or"
...
def test_start_pos():
    ...
    kw = {"positions": True}
    tks = postings.tokens(u"alfa bravo charlie delta", ana, kw)
    assert [t.pos for t in tks] == [0, 1, 2, 3]

    kw["start_pos"] = 3
    ts = [t.copy() for t in postings.tokens(u"A B C D".split(), ana, kw)]
...
def test_frowny_face():
    ...
    ana = tokenizers.RegexTokenizer(r"\S+") | intraword.IntraWordFilter()
...
def test_ngrams():
    ...
    tk = tokenizers.RegexTokenizer(r"\S+")
    ...
    f = ngrams.NgramFilter(3, 4, at="start")
    assert dotest(f) == "abc/abcd/klm"

    f = ngrams.NgramFilter(3, 4, at="end")
...
def test_language_analyzer():
    ...
    for lang, source, target in domain:
        ana = analyzers.LanguageAnalyzer(lang)
...
def test_la_pickleability():
    ana = analyzers.LanguageAnalyzer("en")
    _ = dumps(ana, -1)
...
def test_charset_pickeability():
    ...
    ana = analyzers.StandardAnalyzer() | filters.CharsetFilter(charmap)
    _ = dumps(ana, -1)

    ana = tokenizers.CharsetTokenizer(charmap)
    _ = dumps(ana, -1)
...
def test_shingle_stopwords():
    ...
    ana = (tokenizers.RegexTokenizer() |
           filters.StopFilter(stoplist=None, minsize=3) |
           intraword.ShingleFilter(size=3))
...
@pytest.mark.skipif("sys.version_info < (2,6)")
def test_stop_lang():
    stopper = tokenizers.RegexTokenizer() | filters.StopFilter()
    ls = [token.text for token in stopper(u"this is a test")]
    assert ls == [u"test"]

    es_stopper = tokenizers.RegexTokenizer() | filters.StopFilter(lang="es")
...
def test_ngramwords_tokenizer():
    tk = tokenizers.CommaSeparatedTokenizer()
    tags = fields.NGRAMWORDS(minsize=3, maxsize=50, tokenizer=tk, stored=True)
    schema = fields.Schema(tags=tags)


def test_no_morph():
    ana = analyzers.StemmingAnalyzer()
    value = "calling rendering functions"

    def rt(v, **kwargs):
        return " ".join(t.text for t in ana(v, **kwargs))
