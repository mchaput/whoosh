...
def test_path_tokenizer():
    ...
    t1 = [token.text for token in pt(u'/alfa/bravo/charlie/')]
    assert t1 == ["/alfa", "/alfa/bravo", "/alfa/bravo/charlie"]
    t2 = [token.text for token in pt(u'/home/user/file.txt')]
    assert t2 == ["/home", "/home/user", "/home/user/file.txt"]
...
def test_name_field():
    ana = (analysis.RegexTokenizer(r"\S+")
           | analysis.LowercaseFilter()
           | analysis.DoubleMetaphoneFilter(combine=False))

    t1 = [token.text for token in ana(u"Leif Ericson")]
    t2 = [token.text for token in ana(u"leaf eriksen")]
    assert t1 == t2

    ana = (analysis.RegexTokenizer(r"\S+")
           | analysis.LowercaseFilter()
           | analysis.DoubleMetaphoneFilter(combine=True))
    ts = [token.text for token in ana(u"Leif Ericson")]
    assert ts == ["leif", "LF", "ericson", "ARKSN"]
